    ...
    switch (action.type) {
        case FOLLOW:
            return {
                ...
                // из объектов поменять то нам такой вариант не подходит users: [...state.users]
                // нужно пробежаться по массиву объектов и сделать копию того объекта который мы будем
                // менять return {...u, followed: true} кого менять мы узнаем ч/з action.userId который придет из UI
